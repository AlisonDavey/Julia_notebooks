...
function clean(text)
	...
	filter(isinalphabet, unaccent(map(lowercase, text)))
end
...
# ╔═╡ 5204461e-0350-11eb-01ae-e921d5026a63
filter(alphabet) do letter
	sample_freqs[index_of_letter(letter)]==0.
end
...
double_letters = filter(alphabet) do letter
	sample_freq_matrix[index_of_letter(letter), index_of_letter(letter)]>0.
end
...
most_likely_to_follow_w = filter(alphabet) do elem
sample_freq_matrix[index_of_letter('w'),index_of_letter(elem)]==maximum(sample_freq_matrix[index_of_letter('w'), :])
end
...
most_likely_to_precede_w = filter(alphabet) do elem
sample_freq_matrix[index_of_letter(elem),index_of_letter('w')]==maximum(sample_freq_matrix[:,index_of_letter('w')])
end
...
# ╔═╡ 9076fef2-0353-11eb-0b80-79f3e5349330
sum(sample_freq_matrix,dims=2) # sum rows

# ╔═╡ cb78ceea-03e9-11eb-1e95-d7a39df04803
sum(sample_freq_matrix,dims=1) # sum cols
...
# ╔═╡ 6ef4227a-03d3-11eb-198a-b572b6f42289
matrix_distance(transition_frequencies(mystery_sample),transition_frequencies(samples[1]))
...
# ╔═╡ 2c018d06-03ef-11eb-3edb-45fcb208550d
function word_counts(words::Vector)
	counts = Dict()
	[haskey(counts, word) ? counts[word] += 1 : counts[word] = 1 for word in words]
	return counts
end
...
# ╔═╡ 9472c772-03f1-11eb-1e82-1f3bb366826f
function completions_cache(grams)
	cache = Dict()
	n = length(first(grams))
	[haskey(cache, gram[1:n-1]) ? cache[gram[1:n-1]] = vcat(cache[gram[1:n-1]], gram[n]) : cache[gram[1:n-1]] = [gram[n]] for gram in grams]
	cache
end
...
completions_cache(ngrams_circular(forest_words, 4))
