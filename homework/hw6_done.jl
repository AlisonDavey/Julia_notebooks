...
# ╔═╡ c882d404-1d5e-11eb-2d54-b51b89e3688f
function tangent_line(f::Function, a, h=1e-3)
	# y = f(a) + f'(a)(x-a)
	g = x -> f(a) + finite_difference_slope(f,a,h)*(x-a)
    return g
end
...
function euler_integrate(fprime::Function, fa::Number, T::AbstractRange)
	...
	for t in T
		f = euler_integrate_step(fprime, f, t, step(T))
		push!(lst,f)
	end
...
rapid growth in number of infected then decline as fewer susceptibles
...
md"β = $(@bind guess_β1 Slider(0.00:0.0001:0.2; default = 0.05, show_value=true))"

# ╔═╡ 1761ca0e-1e26-11eb-20c0-1f6a9176611c
md"γ = $(@bind guess_γ1 Slider(0.00:0.0001:0.1; default = 0.05, show_value=true))"

# ╔═╡ 71a27d62-1e26-11eb-2853-cddfd4ba4336
plot_sir!(plot(), sir_T, euler_SIR(guess_β1, guess_γ1, [0.99, 0.01, 0.00], sir_T))
...
function ∂x(f::Function, a, b; h=1e-3 )
	f_x = x -> f(x,b)
	return finite_difference_slope(f_x, a, h)
end
...
function ∂y(f::Function, a, b; h=1e-3 )
	f_y = y -> f(a,y)
	return finite_difference_slope(f_y, b, h)
end
...
function gradient(f::Function, a, b)
	return [∂x(f, a, b; h=1e-3 ), ∂y(f, a, b; h=1e-3 )]
end
...
function gradient_descent_1d_step(f, x0; η=0.01)	
	return x0 - η*finite_difference_slope(f, x0, η)
end
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	chg_x,chg_y=gradient(f, x0, y0)
	return [x0-η*chg_x,y0-η*chg_y]
end
...
# The 4 minima are: (4,-2), (-4,-4), (-3,3), (3,3)
...
# ╔═╡ da864ae8-1e01-11eb-3f52-292fe3c76bf9


# ╔═╡ e36af136-1e01-11eb-2d65-b3a051c8c7d4
...
function loss_dice(μ, σ)
	...
	for i in 1:length(dice_x)
		loss += (gauss(dice_x[i], μ, σ) - dice_y[i])^2
	end
	loss
end
...
# ╔═╡ 9c4c1ecc-1e03-11eb-3e84-d517da0ab583
found_μ, found_σ = let
...
function loss_sir(β, γ)
	loss = 0
	sir = euler_SIR(β, γ, [0.99, 0.01, 0.00], 1:length(hw4_results))
	for i in 1:length(hw4_results)
		loss += first(sum(abs.(hw4_results[i]-sir[i])))
	end
	loss
end
...
# ╔═╡ 5f37d78c-1e25-11eb-0189-57813f5a7a73
guess_β, guess_γ
...
found_β, found_γ = let
	gradient_descent_2d(loss_sir, guess_β, guess_γ; η=1e-9)
end
...
# ╔═╡ 40f4df90-1e27-11eb-1ca8-1bec56eba205
let
