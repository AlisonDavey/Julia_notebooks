...
before 1029 after 687 due to vcat
"""
...
function greedy_seam(energies, starting_pixel::Int)
	...
	js[1] = starting_pixel
	for i=2:m
		j1, j2 = max(1, js[i-1]-1), min(js[i-1]+1, n)
		js[i] = js[i-1]-2+findmin(@view energies[i, j1:j2])[end] + 1*(js[i-1]==1)
	end
	return js
end
...
# ╔═╡ e1bb805a-f677-11ea-2372-df15fd3e58ff
...
function least_energy(energies, i, j)
	...
	js = j .+ [-1,0,1]
	js[1], js[3] = max(1, js[1]), min(js[3], n)
	
	idx,(min_energy,next_loc) = reduce((x,y) -> y[2][1] < x[2][1] ? y : x, 
						         enumerate([least_energy(energies, i+1, js[1]),
						  					least_energy(energies, i+1, js[2]),
						  					least_energy(energies, i+1, js[3])
										   ]))
	...
	return (energies[i,j] + min_energy, js[idx])
end

# ╔═╡ ee667a76-f677-11ea-1975-7b8cea947c2f
function least_energy_matrix_(energies)
	m, n = size(energies)
	least_E = zeros(size(energies))
	dirs = zeros(Int, size(energies))
	least_E[end, :] .= @view energies[end, :] 
	...
	for i in m-1:-1:1, j in 1:n
			...
			e, dir = findmin(@view least_E[i+1, j1:j2])
			least_E[i,j] += e + energies[i,j]
			dirs[i, j] = (-1,0,1)[dir + (j==1)]
	end
	(least_E, dirs)
end

# ╔═╡ 05a47ab0-f678-11ea-3ef4-31cadc21070b
function least_energy_(energies,i,j)
	least_E, dirs = least_energy_matrix_(energies)
	(least_E[i,j],dirs[i,j]+j)
end
...
function recursive_seam(energies, starting_pixel)
	...
	js[1]=starting_pixel
	for k in 1:m-1
		_, js[k+1] = least_energy(energies,k,js[k])
	end
	js
end
...
It's an exhaustive search because `least_energy` is called at every pixel in the path for each loop, having previously already calculated min energy at the same pixels. 
...
# ╔═╡ f61bb8e2-f679-11ea-2c07-fd74eabfe504
begin
	function f(i)
		return i*i
	end
	D=Dict(i => f(i) for i = 1:10)
end

# ╔═╡ f87af6ca-f679-11ea-1725-d109a460d06d
D[5] # value of the key 5 if it exists

# ╔═╡ fed7737a-f679-11ea-0c50-91bc97df3a5d
haskey(D, 5) # has a mapping for a given key

# ╔═╡ 087281cc-f67a-11ea-2005-5f06d24e3e2b
haskey(D, 11)

# ╔═╡ 0acd4486-f67a-11ea-0392-b1b58ea189ef
D_ = Dict('a'=>2, 'b'=>3)

# ╔═╡ 14726214-f67a-11ea-02a9-714be6fa228e
typeof(D_)
...
function memoized_least_energy(energies, i, j, memory)
	...
	if !haskey(memory, (i,j))
			memory[i,j]=least_energy(energies,i,j)[1]
	end
	memory[i,j]
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for k in 2:m
		j1, j2 = max(1, js[k-1]-1), min(js[k-1]+1, n)
		_, dir = findmin((matrix_memoized_least_energy(energies, k, j1, memory),matrix_memoized_least_energy(energies, k, j1+1, memory),matrix_memoized_least_energy(energies, k, j2, memory)))
		js[k] = js[k-1]+(-1,0,1)[dir + (js[k-1]==1)]
	end
...
function least_energy_matrix(energies)
	...
	for i in m-1:-1:1, j in 1:n
			j1, j2 = max(1, j-1), min(j+1, n)
			least_E[i,j] += findmin(@view least_E[i+1, j1:j2])[1] + energies[i,j]	
	end
	least_E
end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	E = least_energy_matrix(energies)
	m, n = size(E)
	js = zeros(Int, m)
	js[1] = starting_pixel
	
	for i = 1:m-1
		j1, j2 = clamp(js[i]-1, 1, n), clamp(js[i]+1, 1, n) 
		js[i+1] = js[i] + (-1,0,1)[(js[i]==1) + findmin(@view E[i+1, j1:j2])[2]]
	end
	js
end
...
# time this is run..1. 439 s .1: 171s 
...
# ╔═╡ 1f8d373e-f678-11ea-08ea-c9f349fa3ee0
...
# ╔═╡ 90877dae-f679-11ea-2bd8-8d80ac520234
@benchmark shrink_n(pika, 3, recursive_seam)[2]
...
# ╔═╡ e74abf3a-f682-11ea-2350-d3a9409924a0
matrix_memoized_seam(energy(pika), 4)
...
# ╔═╡ 4eba6b64-f69e-11ea-2afb-cfb15e56648c
least_energy_matrix(energy(pika))

# ╔═╡ e0b5b176-f681-11ea-22f4-4fcb702dd94f
seam_from_precomputed_least_energy(energy(pika), 4)

# ╔═╡ 3a66fcda-f67f-11ea-28e7-932f646316a0
@benchmark seam_from_precomputed_least_energy(energy(pika), 4)
