...
# ╔═╡ 4f4d607e-14b5-11eb-2c3d-6f775fc1738e
using Statistics
...
# ╔═╡ 12421660-14ac-11eb-06af-354ca7433d11
recovery_time(1.)
...
# ╔═╡ 0946aa44-1ace-11eb-2aae-7546cc690d06
...
# ╔═╡ b792ac26-14af-11eb-072d-c10e15364e96


# ╔═╡ 9462c0e8-14ae-11eb-1052-69b360e922ae
frequencies([7, 8, 9, 7])

# ╔═╡ 66f646a6-14b4-11eb-1ea6-3716aa88b9ce
small_experiment
...
# ╔═╡ c5e545b8-14b4-11eb-320b-cf1028e1b340
do_experiment(0.25, 10)
...
function frequencies_plot_with_mean(data)
	...
	base = bar(frequencies(data), label="frequency")
...
# ╔═╡ c40f1af6-14b5-11eb-2c74-37041bdb3096
@bind p_interactive Slider(0.01:0.01:1., show_value=true)

# ╔═╡ cdb60822-1ac2-11eb-06c2-c3196cd5ba5c
@bind N_interactive Slider(1:1:100_000, show_value=true)

# ╔═╡ a83f9936-1ac3-11eb-2d31-855b510afa6f
...
begin
	x=1:15
	plot!(x,exp.(-x.*p_interactive.-1), label="exponential")
end
...
# ╔═╡ 9c705b2a-1acf-11eb-3a8b-c1b4011fe49c
N = 10_000

# ╔═╡ 53024302-1acf-11eb-365f-fb5735e5496c
p_range = 0.001:0.001:1

# ╔═╡ 7335de44-042f-11eb-2873-8bceef722432
function mean_time_to_recover(N)
	m_times = []
	for (_,p) in enumerate(p_range)
		data = do_experiment(p,N)
		push!(m_times, sum(data)/N)
	end
	return m_times
end

# ╔═╡ 7b671740-1ac8-11eb-19cc-13e347579011
begin
	m_time_plot = plot(p_range,
		mean_time_to_recover(N),
		legend = false,
		xlabel = "Probability (p)",
		ylabel = "Mean Time to recover (days)",
		xlim=(0,1),
		ylim=(0,250))
end
...
# ╔═╡ e8dadc12-1ad0-11eb-040e-4795cc225759
Integer(test_status)

# ╔═╡ f7bde1e0-1ad0-11eb-1d4b-5d013584e078
Integer(I)

# ╔═╡ fe5d4c84-1ad0-11eb-13b7-519a7c560fa7
Integer(R)
...
# ╔═╡ 7bf7799a-1ad0-11eb-2bde-f193cc136f63
...
# ╔═╡ 61873b58-1ad1-11eb-0dad-31a54625aa73
test_agent.status

# ╔═╡ 6a7dba5c-1ad1-11eb-2044-fb45ec5ea4bf
test_agent.num_infected
...
# ╔═╡ c7702d34-1ad2-11eb-0e56-11c3d700a154
function set_num_infected!(agent::Agent, new_num_infected::Integer)
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	mean_S = sum(a -> a.S, simulations)/N
	mean_I = sum(a -> a.I, simulations)/N
	mean_R = sum(a -> a.R, simulations)/N
...
@bind p_infection Slider(0.001:0.001:.2, show_value=true)

# ╔═╡ f8499da4-1ba5-11eb-166f-ab5bcfa3e23b
@bind p_recovery Slider(0.001:0.001:.2, show_value=true)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	std_S = (((sum(a -> a.S, simulations) .- mean_S).^2)./(N-1)).^0.5
	std_I = (((sum(a -> a.I, simulations) .- mean_I).^2)./(N-1)).^0.5
	std_R = (((sum(a -> a.R, simulations) .- mean_R).^2)./(N-1)).^0.5
	...
	plot!(p, mean_S, linewidth=3, yerr=std_S, label="mean S", alpha=0.01)
	plot!(p, mean_I, linewidth=3, yerr=std_I, label="mean I", alpha=0.01)
	plot!(p, mean_R, linewidth=3, yerr=std_R, label="mean R", alpha=0.01)
...
Over the course of the simulation, we accumulate the information about how many other agents each agent has infected

At the end of the simulation we look at the probability distribution of "the number of agents infected by an individual"
...
let
	agent = Agent(S, 0)
	source = Agent(I, 0)
	infection = InfectionRecovery(0.4, 0.5)
...
function sweep!(agents::Vector{Agent}, infection::AbstractInfection)
	map(1:length(agents)) do _
		step!(agents, infection)
	end
	return agents
end

# ╔═╡ ea9f7714-1ba0-11eb-10e9-f3e4ca9632bc
sweep!(generate_agents(5), InfectionRecovery(0.9, 0.2))
...
let
	...
	mean_I = sum(a -> a.I, simulations)./length(simulations)
	plot!(p, mean_I, linewidth=3, label="mean I", ylabel = "# of Infectious Agents")
	p
end
...
# ╔═╡ 3aadb856-1ba6-11eb-04b4-856cd88271da
simulations_p = repeat_simulations(100, 
	1000,
	InfectionRecovery(p_infection, p_recovery),
	20)

# ╔═╡ 4e733702-1ba7-11eb-0bb2-6bf4f4f53262
sir_mean_plot(simulations_p)

# ╔═╡ 5c352842-1ba6-11eb-339e-416eb501c90d
sir_mean_error_plot(simulations_p)

# ╔═╡ 26e2978e-0435-11eb-0d61-25f552d2771e
function num_infected_simulation(N::Integer, T::Integer, infection::AbstractInfection)
	agents = generate_agents(N)
	map(1:T) do _
		sweep!(agents, infection)
	end
	return [agents[i].num_infected for i in 1:length(agents)]
end

# ╔═╡ ff38765c-1bce-11eb-36af-bf3982877525
function repeat_num_infected_simulations(N, T, infection, num_simulations)
	map(1:num_simulations) do _
		num_infected_simulation(N, T, infection)
	end
end

# ╔═╡ 986c4a84-1bce-11eb-0091-3305fefd23b2
bar(frequencies(collect(Iterators.flatten(repeat_num_infected_simulations(100,1000,InfectionRecovery(0.02, 0.002),20)))), label=nothing)

# ╔═╡ 4ad11052-042c-11eb-3643-8b2b3e1269bc
begin
	agents = generate_agents(100)
	map(1:1000) do _
		sweep!(agents, InfectionRecovery(0.01, 0.002))
	end
end

# ╔═╡ fb39c20e-1c34-11eb-3c34-3bac01289b1a
# Number of people who were infected (1 person initially infected) during simulation
sum(a -> a.num_infected, agents)+1 

# ╔═╡ e7d24c26-1c33-11eb-0368-5ddb18bb93da
# Number of people who had recovered by end of simulation
sum(a -> a.status==R, agents)

# ╔═╡ 092efe96-1c34-11eb-2d90-99829e0d34cb
# Number of people infected or recovered by end of simulation
sum(a -> (a.status==R) | (a.status==I), agents)
...
# ╔═╡ 1894ee5e-1c43-11eb-2411-f5c5b8ae15e3
simulations_r = repeat_simulations(100, 1000, Reinfection(0.05, 0.01), 20)
...
function sir_reinfection_mean_I_plot(simulations::Vector{<:NamedTuple})
	T = length(first(simulations).S)
	N = length(simulations)
	mean_I = sum(a -> a.I, simulations)/N
	plot(mean_I,
	...
	plot!(sum(a -> a.S, simulations)/N, label="Susceptible")
end		
	

# ╔═╡ 31f1d6dc-1c43-11eb-2e79-47a82bf0842c
sir_reinfection_mean_I_plot(simulations_r)
